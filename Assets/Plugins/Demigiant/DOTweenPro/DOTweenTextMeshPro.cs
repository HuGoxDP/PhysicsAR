    ...
    public class DOTweenTMPAnimator : IDisposable
    {
        ...
        struct CharTransform
        {
            ...
            public CharVertices GetVertices()
            {
                return new CharVertices(
                    _meshInfo.vertices[_firstVertexIndex],
                    _meshInfo.vertices[_firstVertexIndex + 1],
                    _meshInfo.vertices[_firstVertexIndex + 2],
                    _meshInfo.vertices[_firstVertexIndex + 3]
                );
            }
        ...
        public TweenerCore<Color, Color, ColorOptions> DOFadeChar(int charIndex, float endValue, float duration)
        {
            ...
            TweenerCore<Color, Color, ColorOptions> t = DOTween.ToAlpha(
                ...
                x => { _charTransforms[charIndex].UpdateAlpha(target, x, textInfo.meshInfo); },
                endValue,
                duration
            );
            return t;
        ...
        public TweenerCore<Color, Color, ColorOptions> DOColorChar(int charIndex, Color endValue, float duration)
        {
            ...
            TweenerCore<Color, Color, ColorOptions> t = DOTween.To(
                () => _charTransforms[charIndex].GetColor(textInfo.meshInfo),
                x => { _charTransforms[charIndex].UpdateColor(target, x, textInfo.meshInfo); },
                endValue,
                duration
            );
            return t;
            ...
            float duration)
        {
            if (!ValidateChar(charIndex)) return null;
            TweenerCore<Vector3, Vector3, VectorOptions> t = DOTween.To(
                () => _charTransforms[charIndex].offset,
                x =>
                {
            ...
            float duration, RotateMode mode = RotateMode.Fast)
        {
            ...
            TweenerCore<Quaternion, Vector3, QuaternionOptions> t = DOTween.To(
                () => _charTransforms[charIndex].rotation,
                x =>
                {
                    CharTransform charT = _charTransforms[charIndex];
        ...
        public TweenerCore<Vector3, Vector3, VectorOptions> DOScaleChar(int charIndex, Vector3 endValue, float duration)
        {
            if (!ValidateChar(charIndex)) return null;
            TweenerCore<Vector3, Vector3, VectorOptions> t = DOTween.To(
                () => _charTransforms[charIndex].scale,
                x =>
                {
                    CharTransform charT = _charTransforms[charIndex];
            ...
            float elasticity = 1)
        {
            ...
            return DOTween.Punch(
                () => _charTransforms[charIndex].rotation.eulerAngles,
                x =>
                {
                    CharTransform charT = _charTransforms[charIndex];
