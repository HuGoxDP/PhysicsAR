...
#pragma warning restore 414
        ...
        static readonly GUIContent _GuiC_selfTarget_true = new GUIContent(
            "SELF",
            "Will animate components on this gameObject"
        );

        static readonly GUIContent _GuiC_selfTarget_false = new GUIContent(
            "OTHER",
            "Will animate components on the given gameObject instead than on this one"
        );
        ...
        override public void OnInspectorGUI()
        {
            ...
            GUILayout.Label(
                _src.animationType.ToString() + (string.IsNullOrEmpty(_src.id) ? "" : " [" + _src.id + "]"),
            ...
            if (playMode)
            {
                ...
                else
                {
                    ...
                    {
                        GUILayout.Label(
                            "This animation has been toggled as inactive and won't be generated",
...
//            _src.isValid = Validate(); // Moved down
            ...
            else
            {
                ...
                _settings.showPreviewPanel = hasManager
                    ? DeGUILayout.ToggleButton(
                        _settings.showPreviewPanel,
                        "Preview Controls",
            ...
            // Choose target
            GUILayout.BeginHorizontal();
            _src.isActive = EditorGUILayout.Toggle(
                new GUIContent("", "If unchecked, this animation will not be created"),
                _src.isActive,
                GUILayout.Width(14)
            );
            EditorGUI.BeginChangeCheck();
            ...
            _src.targetIsSelf = DeGUILayout.ToggleButton(
                _src.targetIsSelf,
                _src.targetIsSelf ? _GuiC_selfTarget_true : _GuiC_selfTarget_false,
                new Color(1f, 0.78f, 0f),
                DeGUI.colors.bg.toggleOn,
                new Color(0.33f, 0.14f, 0.02f),
...
//                _src.animationType = (DOTweenAnimation.AnimationType)EditorGUILayout.EnumPopup(_src.animationType, EditorGUIUtils.popupButton);
                ...
                _src.animationType = AnimationToDOTweenAnimationType(
                    _AnimationType[EditorGUILayout.Popup(
                        DOTweenAnimationTypeToPopupId(_src.animationType),
                        _AnimationType
                    )]
                );
                _src.autoGenerate = DeGUILayout.ToggleButton(
                    _src.autoGenerate,
                    new GUIContent(
                        "AutoGenerate",
                ...
                {
                    _src.autoPlay = DeGUILayout.ToggleButton(
                        _src.autoPlay,
                        new GUIContent("AutoPlay", "If selected, the tween will play automatically")
                    );
                }

                _src.autoKill = DeGUILayout.ToggleButton(
                    _src.autoKill,
                    new GUIContent(
                        "AutoKill",
                ...
                if (_chooseTargetMode == ChooseTargetMode.BetweenCanvasGroupAndImage &&
                    _src.forcedTargetType != DOTweenAnimation.TargetType.Unset)
                {
                    FadeTargetType fadeTargetType = (FadeTargetType)Enum.Parse(
                        typeof(FadeTargetType),
                        _src.forcedTargetType.ToString()
                    );
                ...
                _src.loops = EditorGUILayout.IntField(
                    new GUIContent("Loops", "Set to -1 for infinite loops"),
                    _src.loops
                );
                ...
                switch (_src.animationType)
                {
                    ...
                    case DOTweenAnimation.AnimationType.LocalRotate:
                        ...
                        else
                        {
                            ...
                            _src.optionalRotationMode = (RotateMode)EditorGUILayout.EnumPopup(
                                "    Rotation Mode",
                    ...
                    case DOTweenAnimation.AnimationType.Text:
                        ...
                        _src.optionalString = EditorGUILayout.TextField(
                            new GUIContent(
                                "Custom Scramble",
                                "Custom characters to use in case of ScrambleMode.Custom"
                            ),
                            _src.optionalString
                        );
                        break;
                    ...
                    case DOTweenAnimation.AnimationType.PunchScale:
                        ...
                        _src.optionalInt0 = EditorGUILayout.IntSlider(
                            new GUIContent("    Vibrato", "How much will the punch vibrate"),
                            _src.optionalInt0,
                            1,
                            50
                        );
                    ...
                    case DOTweenAnimation.AnimationType.ShakeScale:
                        ...
                        canBeRelative = false;
                        _src.optionalInt0 = EditorGUILayout.IntSlider(
                            new GUIContent("    Vibrato", "How much will the shake vibrate"),
                            _src.optionalInt0,
                            1,
                            50
                        );
                        ...
                        {
                            _src.optionalFloat0 = EditorGUILayout.Slider(
                                new GUIContent("    Randomness", "The shake randomness"),
                                _src.optionalFloat0,
                                0,
                                90
                            );
                            _src.optionalShakeRandomnessMode = (ShakeRandomnessMode)EditorGUILayout.EnumPopup(
        ...
        void GUIEndValueV3(GameObject targetGO, bool optionalTransform = false)
        {
            ...
            if (_src.useTargetAsV3)
            {
                ...
                if (_src.endValueTransform != prevT && _src.endValueTransform != null)
                {
                    ...
                    if (targetGO.GetComponent<RectTransform>() != null)
                    {
                        if (_src.endValueTransform.GetComponent<RectTransform>() == null)
                        {
                            EditorUtility.DisplayDialog(
                                "DOTween Pro",
                                "For Unity UI elements, the target must also be a UI element",
                                "Ok"
                            );
                            _src.endValueTransform = null;
                        }
                    ...
                    else if (_src.endValueTransform.GetComponent<RectTransform>() != null)
                    {
                        EditorUtility.DisplayDialog(
                            "DOTween Pro",
                            "You can't use a UI target for a non UI object",
                            "Ok"
                        );
                        _src.endValueTransform = null;
                    }
            ...
            {
                if (GUILayout.Button(
                        _src.useTargetAsV3 ? "target" : "value",
                        EditorGUIUtils.sideBtStyle,
                        GUILayout.Width(44)
