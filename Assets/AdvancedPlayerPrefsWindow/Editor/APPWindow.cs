...
namespace RejectedGames
{
    public class APPWindow : EditorWindow
    {
        ...
        public Texture2D DeleteIcon
        {
            get
            {
                if ((Object)_deleteIcon == (Object)null)
                    _deleteIcon =
        ...
        public Texture2D AddIcon
        {
            get
            {
                if ((Object)_addIcon == (Object)null)
        ...
        public Texture2D UndoIcon
        {
            get
            {
                if ((Object)_undoIcon == (Object)null)
        ...
        public Texture2D SaveIcon
        {
            get
            {
                if ((Object)_saveIcon == (Object)null)
        ...
        public Texture2D RefreshIcon
        {
            get
            {
                if ((Object)_refreshIcon == (Object)null)
                    _refreshIcon =
        ...
        {
            get { return GetStyle("ToolbarSearchTextField", "ToolbarSeachTextField"); }
        }

        private GUIStyle ToolbarSeachTextFieldPopup
        {
            get { return GetStyle("ToolbarSearchTextFieldPopup", "ToolbarSeachTextFieldPopup"); }
        }

        private GUIStyle ToolbarSeachCancelButton
        {
            get { return GetStyle("ToolbarSearchCancelButton", "ToolbarSeachCancelButton"); }
        }

        private GUIStyle ToolbarSeachCancelButtonEmpty
        {
            get { return GetStyle("ToolbarSearchCancelButtonEmpty", "ToolbarSeachCancelButtonEmpty"); }
        }
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                if (GUI.Button(position, "", ToolbarSeachTextFieldPopup))
                {
                    ...
                    options.AddItem(
                        new GUIContent("Value (Strings only)"),
                ...
                if (Application.platform == RuntimePlatform.WindowsEditor)
                {
                    ...
                    AutoRefresh = GUILayout.Toggle(
                        AutoRefresh,
                        new GUIContent("Auto Refresh ", refreshTooltip),
                        EditorStyles.toolbarButton,
                        GUILayout.ExpandWidth(false),
                ...
                if (GUILayout.Button(
                        new GUIContent(RefreshIcon, "Force a refresh, could take a few seconds."),
                        EditorStyles.toolbarButton,
                        GUILayout.ExpandWidth(false),
                ...
                if (waitTillPlistHasBeenWritten)
                {
                    Texture2D t = AssetDatabase.LoadAssetAtPath(
                ...
                EditorGUILayout.BeginVertical();
                {
                    for (int i = 0; i < filteredPpeList.Count; i++)
                    {
                        if (filteredPpeList[i].Value != null)
                        {
                            ...
                            {
                                filteredPpeList[i].IsSelected = GUILayout.Toggle(
                                    filteredPpeList[i].IsSelected,
                                ...
                                filteredPpeList[i].Key = EditorGUILayout.TextField(
                                    filteredPpeList[i].Key,
                                    ...
                                    case ValueType.String:
                                        ...
                                        else
                                        {
                                            ...
                                            EditorGUILayout.TextField(
                                                "",
                                                string.Format(
                                                    "Cannot display value as it exceeds the textfield character limitation. Value has {0} characters while Unity only supports {1}.",
                                                    textValue.Length,
                                                    CHARACTER_LIMIT
                                                ),
                                                EditorStyles.numberField,
                                ...
                                GUILayout.Label(
                                    new GUIContent("?", filteredPpeList[i].Type.ToString()),
                                    GUILayout.ExpandWidth(false)
                                );

                                GUI.enabled = filteredPpeList[i].HasChanged && !waitTillPlistHasBeenWritten;
                                if (GUILayout.Button(
                                        new GUIContent(SaveIcon, "Save changes made to this value."),
                                        GUILayout.ExpandWidth(false)
                                    ))
                                {
                                ...
                                if (GUILayout.Button(
                                        new GUIContent(UndoIcon, "Discard changes made to this value."),
                                        GUILayout.ExpandWidth(false)
                                    ))
                                {
                                ...
                                if (GUILayout.Button(
                                        new GUIContent(DeleteIcon, "Delete this key-value."),
                                        GUILayout.ExpandWidth(false)
                                    ))
                                {
        ...
        private string[] GetAllMacKeys()
        {
            ...
            string plistPath = string.Format(
                "{0}/Library/Preferences/unity.{1}.{2}.plist",
 ...
 cuKey.CreateSubKey("Software\\AppDataLow\\Software\\" + PlayerSettings.companyName + "\\" + PlayerSettings.productName);
        ...
        private bool IsUnityWritingToPlist()
        {
            ...
            if (Application.platform == RuntimePlatform.OSXEditor)
            {
                ...
                FileInfo plistFile = new FileInfo(
                    string.Format(
                        "{0}unity.{1}.{2}.plist",
                        preferencesPath,
        ...
        private GUIStyle GetStyle(string styleName, string fallbackStyle)
        {
            ...
            if (guiStyle == null)
            {
                guiStyle = GUI.skin.FindStyle(fallbackStyle) ??
                           EditorGUIUtility.GetBuiltinSkin(EditorSkin.Inspector).FindStyle(fallbackStyle);
            }
            ...
            if (!string.IsNullOrEmpty(importPath))
            {
                ...
                foreach (KeyValuePair<string, object> kvp in plist)
                {
                    ...
                    if (entry != null)
                    {
                        ...
                        else if (!entry.Value.Equals(existingEntry.Value) && EditorUtility.DisplayDialog(
                                     "Duplicate entry found",
                                     string.Format(
                                         "Key \"{0}\" with value \"{1}\" already exists.{2}Do you want to overwrite it with value \"{3}\" ?",
                                         existingEntry.Key,
                                         existingEntry.Value,
                                         Environment.NewLine,
                                         entry.Value
            ...
            if (isOneSelected)
            {
                if (!waitTillPlistHasBeenWritten)
                {
                    if (EditorUtility.DisplayDialog(
                            "Are you sure?",
                            "Are you sure you want to delete the selected keys? There is no undo!",
                            "Delete",
                            "Cancel"
                        ))
                    {
        ...
        private void Export(bool onlySelected)
        {
            ...
            else
            {
                string exportPath = EditorUtility.SaveFilePanel(
                    "Export all PlayPrefs entries",
                    Application.dataPath,
                    PlayerSettings.productName + "_PlayerPrefs",
                    "ppe"
                );
