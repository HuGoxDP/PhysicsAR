...
namespace FullscreenEditor
{
    ...
    public static class FullscreenPreferences
    {
        ...
        private const string ASSET_STORE_PAGE =
        ...
        private static readonly GUIContent resetSettingsContent = new GUIContent(
            "Use Defaults",
            "Reset all settings to default ones"
        );

        private static readonly GUIContent versionContent =
        ...
        private static readonly PrefItem<Vector2> scroll = new PrefItem<Vector2>(
            "Scroll",
            Vector2.zero,
            string.Empty,
            string.Empty
        ...
        static FullscreenPreferences()
        {
            ...
            rectSourceTooltip +=
                "Window Display: Open on the display that the target window is located (Windows only);\n\n";
            ...
            ToolbarVisible = new PrefItem<bool>(
                "Toolbar",
                false,
                "Toolbar Visible",
                "Show and hide the toolbar on the top of some windows, like the Game View and Scene View."
            );
            FullscreenOnPlayEnabled = new PrefItem<bool>(
                "FullscreenOnPlay",
                false,
                "Fullscreen On Play",
                "Override the \"Maximize on Play\" option of the game view to \"Fullscreen on Play\""
            );
            RectSource = new PrefItem<RectSourceMode>(
                "RectSource",
                RectSourceMode.MainDisplay,
                "Placement source",
                rectSourceTooltip
            );
            CustomRect = new PrefItem<Rect>(
                "CustomRect",
                FullscreenRects.GetMainDisplayRect(),
                "Custom Rect",
                string.Empty
            );
            DisableNotifications = new PrefItem<bool>(
                "DisableNotifications",
                false,
                "Disable Notifications",
                "Disable the notifications that shows up when opening a new fullscreen view."
            );
            KeepFullscreenBelow = new PrefItem<bool>(
                "KeepFullscreenBelow",
                true,
            ...
            DisableSceneViewRendering = new PrefItem<bool>(
            ...
            UseGlobalToolbarHiding = new PrefItem<bool>(
                "UseGlobalToolbarHiding",
            ...
            MosaicMapping = new PrefItem<int[]>(
                "MosaicMapping",
                new[] { 0, 1, 2, 3, 4, 5, 6, 7 },
                "Mosaic Screen Mapping",
                "Defines which display renders on each screen when using Mosaic."
            );
            DoNotUseWmctrl = new PrefItem<bool>(
                "DoNotUseWmctrl",
                false,
                "Do not use wmctrl",
                "Avoid using 'wmctrl' helper when opening fullscreen windows"
            );
            RestoreCursorLockAndHideState = new PrefItem<bool>(
                "RestoreCursorLockAndHideState",
                true,
            ...
            if (FullscreenUtility.MenuItemHasShortcut(Shortcut.TOOLBAR_PATH))
                ToolbarVisible.Content.text += string.Format(
                    " ({0})",
                    FullscreenUtility.TextifyMenuItemShortcut(Shortcut.TOOLBAR_PATH)
                );
            if (FullscreenUtility.MenuItemHasShortcut(Shortcut.FULLSCREEN_ON_PLAY_PATH))
                FullscreenOnPlayEnabled.Content.text += string.Format(
                    " ({0})",
                    FullscreenUtility.TextifyMenuItemShortcut(Shortcut.FULLSCREEN_ON_PLAY_PATH)
                );
        }
        ...
        {
            var sp = new SettingsProvider(
                "Preferences/Fullscreen Editor/Shortcuts",
                SettingsScope.User,
                contents.Select(c => c.text)
            );
            sp.footerBarGuiHandler = OnFooterGUI;
        ...
        private static void OnPreferencesGUI(string search)
        {
            ...
            if (RectSource.Value == RectSourceMode.AtMousePosition)
                EditorGUILayout.HelpBox(
                    "\'At mouse position\' can cause slowdowns on large projects",
                    MessageType.Warning
                );

            if (!IsRectModeSupported(RectSource))
                EditorGUILayout.HelpBox(
                    "The selected placement source mode is not supported on this platform",
                    MessageType.Warning
                );
            ...
            {
                using (new EditorGUI.DisabledGroupScope(!wmctrl.IsInstalled))
                {
                ...
                {
                    EditorGUILayout.HelpBox(
                        "'wmctrl' not found. Try installing it with 'sudo apt-get install wmctrl'.",
                        MessageType.Warning
                    );
                }
                else
                {
                    EditorGUILayout.HelpBox(
                        "Try enabling the option above if you're experiencing any kind of toolbars or offsets " +
            ...
            if (FullscreenRects.ScreenCount > 1)
            {
                ...
                {
                    var val = EditorGUILayout.IntPopup(
                        string.Format("Physical display {0} renders", i + 1),
                        mosaicMapping[i],
        ...
        private static void OnFooterGUI()
        {
            Func<GUIContent, bool> linkLabel = (label) =>
                typeof(EditorGUILayout).HasMethod("LinkLabel", new[] { typeof(string), typeof(GUILayoutOption[]) })
                    ? // Issue #100
                    typeof(EditorGUILayout).InvokeMethod<bool>("LinkLabel", label, new GUILayoutOption[0])
                    : // < 2020.1
                    typeof(EditorGUILayout).InvokeMethod<bool>(
                        "LinkButton",
                        label,
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (linkLabel(
                        new GUIContent("Consider leaving a review if you're enjoying Fullscreen Editor!", REVIEWS_LINK)
                    ))
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    GUI.changed = false;
                    var enable = GUILayout.Toggle(
                        Integration.IsDirectiveDefined("FULLSCREEN_DEBUG"),
                        "Debug",
                        "Button"
                    );
                    if (GUI.changed)
        ...
        private static string GetEmailURL(Exception e = null)
        {
            ...
            Func<string, string> EscapeURL = url => UnityWebRequest.EscapeURL(url).Replace("+", "%20");
#else
