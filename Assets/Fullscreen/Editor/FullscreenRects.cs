...
namespace FullscreenEditor
{
    ...
    public static class FullscreenRects
    {
        ...
        {
            if (targetWindow != null && !targetWindow.IsOfType(typeof(EditorWindow)) &&
                !targetWindow.IsOfType(Types.View))
            {
                throw new ArgumentException(
                    "Target window must be of type EditorWindow or View or null",
                    "targetWindow"
                );
            }
            ...
            switch (mode)
            {
                ...
                default:
                    Logger.Warning(
                        "Invalid fullscreen mode, please fix this by changing the placement source mode in preferences."
                    );
        ...
        {
            return Types.ContainerWindow.InvokeMethod<Rect>(
                "FitRectToScreen",
                new Rect(Vector2.zero, Vector2.one * 10000f),
                true,
                mouseScreen
            );
        }
        ...
        {
            return container.InvokeMethod<Rect>(
                "FitWindowRectToScreen",
                new Rect(Vector2.zero, Vector2.one * 10000f),
                true,
                mouseScreen
            );
