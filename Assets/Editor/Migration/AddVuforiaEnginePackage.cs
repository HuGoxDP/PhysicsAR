...
public class AddVuforiaEnginePackage
{
    ...
    {
        var tarFilePaths = Directory.GetFiles(Path.Combine(Directory.GetCurrentDirectory(), VUFORIA_TAR_FILE_DIR))
    ...
    class Manifest
    {
            ...
            if (ScopedRegistries.Length > 0)
                return JsonUtility.ToJson(
                    new UnitySerializableManifest
                        { scopedRegistries = ScopedRegistries, dependencies = new DependencyPlaceholder() },
                    true
                );
        ...
        public static Manifest JsonDeserialize(string path)
        {
            ...
            var registries = JsonUtility.FromJson<UnitySerializableManifest>(jsonString).scopedRegistries ??
                             new ScopedRegistry[0];
