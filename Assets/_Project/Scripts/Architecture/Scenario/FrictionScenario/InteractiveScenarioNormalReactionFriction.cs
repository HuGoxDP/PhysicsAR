...
{
    public class InteractiveScenarioNormalReactionFriction : InteractiveScenario
    {
        ...
        [SerializeField] private List<FrictionItem> _itemList;
        ...
        private ObservableFieldComponent<int> _boxCount;

        // Physics
        private ObservableFieldComponent<float> _weight;
        ...
        private void UpdatePhysics()
        {
            ...
            if (_currentAppliedForce.Value >= _staticFrictionMax.Value)
            {
                ...
                var acceleration = netForce / _weight.Value;
        ...
        private void InitializeMaterials()
        {
            ...
            _boxCount = new ObservableFieldComponent<int>(
                1,
                ObservableFieldComponentType.Dropdown,
                "Кількість коробок"
            );

            var boxCountOptions = new List<int>();

            for (int i = 1; i <= _itemList.Count; i++)
            {
                boxCountOptions.Add(i);
            }

            _boxCount.SetAvailableOptions(boxCountOptions);
        }
        ...
        {
            _weight = new ObservableFieldComponent<float>(
                1f,
                ObservableFieldComponentType.None,
                "Вага коробки(кг)",
                "{0:0.#}",
        ...
        private void InitializeRestrictions()
        {
            ...
            _weight.AddRestriction(greaterOrEqualRestriction);
            _gravity.AddRestriction(greaterOrEqualRestriction);
        }

        private void InitializeEvents()
        {
            _boxCount.ValueObservable.DistinctUntilChanged().Subscribe(boxCount =>
                {
                    OnMaterialChanged();
                    for (int i = 0; i < _itemList.Count; i++)
                    {
                        _itemList[i].ItemGameObject.SetActive(i < boxCount);
                    }

                    _weight.TrySetValue(boxCount * 1f);
                }
            ).AddTo(this);

            _weight.ValueObservable
