...
namespace VFavorites
{
    public static class VFavorites
    {
        ...
        static Delegate origBrowserOnGUIDelegate;
        ...
        static Type t_VHierarchy = Type.GetType("VHierarchy.VHierarchy") ?? Type.GetType(
            "VHierarchy.VHierarchy, VHierarchy, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null"
        );

        static Type t_VFolders = Type.GetType("VFolders.VFolders") ?? Type.GetType(
            "VFolders.VFolders, VFolders, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null"
        );

        static Type t_VTabs = Type.GetType("VTabs.VTabs") ??
                              Type.GetType("VTabs.VTabs, VTabs, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null");

        static MethodInfo mi_VHierarchy_GetIconName =
            t_VHierarchy?.GetMethod("GetIconName_forVFavorites", maxBindingFlags);

        static MethodInfo mi_VFolders_DrawBigFolderIcon =
            t_VFolders?.GetMethod("DrawBigFolderIcon_forVFavorites", maxBindingFlags);

        static float rowHeight => 44 * data.rowScale;

        static Rect totalRect_browserSpace => isOneColumn
            ? wrappedBrowser.position.SetPos(0, 0)
            : wrappedBrowser.GetFieldValue<Rect>("m_TreeViewRect");
        ...
        static Vector2 mousePosition_rowsSpace =>
            mousePosiion_browserSpace.AddY(groupRectOffsetY + data.curPage.scrollPos);

        static Vector2 mouseDownPosition_groupSpace => mouseDownPosiion_browserSpace.AddY(groupRectOffsetY);

        static Vector2 mouseDownPosition_rowsSpace =>
            mouseDownPosiion_browserSpace.AddY(groupRectOffsetY + data.curPage.scrollPos);
        ...
        static float targetOpacity =>
            curEvent.holdingAlt || renamingPage || draggingItemFromPageToOutside || isWrappedBrowserLocked
                ? 1
                : 0; // holdingAlt instead of shortcutPressed to prevent unwrapping due to incorrect event modifiers on key down on mac

        static bool draggingItemFromOutside => draggingItem && !draggingItemFromPage;

        static float draggedItemY_groupSpace =>
            (mousePosition_groupSpace.y - rowHeight / 2 + draggedItemHoldOffset).Clamp(0, 12321);

        static float draggedItemY_rowsSpace => draggedItemY_groupSpace + data.curPage.scrollPos;

        static int insertDraggedItemAtIndex => ((mousePosition_rowsSpace.y + draggedItemHoldOffset) / rowHeight)
            .FloorToInt().Clamp(0, data.curPage.items.Count);
        ...
        static IEnumerable<EditorWindow> allBrowsers => _allBrowsers ??=
            t_BrowserWindow.GetFieldValue<IList>("s_ProjectBrowsers").Cast<EditorWindow>();

        static void WrappedOnGUI(object _)
        {
            if (wrappedBrowser?.GetType() != t_BrowserWindow)
            {
                originalBrowserGUI();
                return;
            }
            ...
            void createData()
            {
                ...
                AssetDatabase.CreateAsset(
                    data,
                    GetScriptPath("VFavorites").GetParentPath().CombinePath("vFavorites Data.asset")
                );
                ...
                var guidsFromV1 = EditorPrefs.GetString("vFavorites-guids-" + GetProjectId()).Split('-')
                    .Where(r => r != "").ToList();
            ...
            void pages()
            {
                void page(Rect pageRect, Page page)
                {
                    void findSelectedItem()
                    {
                        ...
                        foreach (var item in page.items)
                        {
                            ...
                            if (item.isFolder)
                            {
                                var targetBrowser = isWrappedBrowserLocked && isOneColumn
                                    ? allBrowsers.FirstOrDefault(r => !r.GetMemberValue<bool>("isLocked")) ??
                                      wrappedBrowser
                                    : wrappedBrowser;

                                if (targetBrowser.GetFieldValue<int>("m_ViewMode") == 1)
                                    isSelected = targetBrowser.InvokeMethod<string>("GetActiveFolderPath") ==
                                                 item.assetPath;
                                else
                    ...
                    void rows()
                    {
                        void row(float y, Item item)
                        {
                            ...
                            void icon()
                            {
                                ...
                                void asset()
                                {
                                    ...
                                    var iconTexture = item.isLoadable
                                        ? AssetPreview.GetAssetPreview(item.obj) ??
                                          AssetPreview.GetMiniThumbnail(item.obj)
                                        : AssetPreview.GetMiniTypeThumbnail(item.type);
                                ...
                                void sceneGameObject()
                                {
                                    ...
                                    void getIconNameFromVHierarchy()
                                    {
                                        ...
                                        var iconNameFromVHierarchy = (string)mi_VHierarchy_GetIconName.Invoke(
                                            null,
                                    ...
                                    var iconTexture = EditorGUIUtility.IconContent(
                                        item.sceneGameObjectIconName.IsNullOrEmpty()
                                            ? "GameObject icon"
                                            : item.sceneGameObjectIconName
                                    ).image;
                                ...
                                void folder()
                                {
                                    ...
                                    {
                                        mi_VFolders_DrawBigFolderIcon?.Invoke(
                                            null,
                                            new object[] { iconRect, item.globalId.guid }
                                        );
                                    }
                            ...
                            void name()
                            {
                                var nameRect = rowRect.MoveX(iconOffset + iconSize + nameOffset).MoveY(-.5f)
                                    .SetHeightFromMid(16);
                                ...
                                void highlighted()
                                {
                                    ...
                                    GUI.skin.GetStyle("WhiteLabel").Draw(
                                        nameRect,
                                        item.name,
                                        false,
                                        false,
                            ...
                            {
                                var labelRect = rowRect.MoveX(
                                    iconOffset + iconSize + nameOffset + item.name.GetLabelWidth() +
                                    deletedOrNotLoadedLabelOffset
                                ).MoveY(.5f);
                            ...
                            void crossButton()
                            {
                                ...
                                var buttonRect = rowRect.SetWidthFromRight(0).MoveX(-crossButtonOffsetFromRight)
                                    .SetWidthFromMid(crossButtonSize);
                                ...
                                SetGUIColor(
                                    buttonRect.IsHovered() ? (mousePressed ? pressedColor : hoveredColor) : normalColor
                                );
                        ...
                        if (curEvent.isRepaint && skipNextRepaint)
                        {
                ...
                for (int i = pagesScrollPos.FloorToInt(); i <= pagesScrollPos.CeilToInt(); i++)
                    page(
                        totalRect_groupSpace.SetX(
                            (totalRect_groupSpace.width + spaceBetweenPages) * (i - pagesScrollPos)
                        ),
                        data.pages[i]
                    );
            }
            ...
            void widget()
            {
                ...
                widgetRect_groupSpace = totalRect_groupSpace
                    .SetWidthFromMid(data.curPage.name.GetLabelWidth(textSize) + widthToAdd).SetHeightFromBottom(height)
                    .MoveY(-distToBottom);
                ...
                void nameLabel()
                {
                    ...
                    var buttonRect =
                        widgetRect_groupSpace.SetWidthFromMid(data.curPage.name.GetLabelWidth(textSize) - 2);
                ...
                void leftButton()
                {
                    ...
                    var iconRect = widgetRect_groupSpace.SetWidth(chevronOffset * 2)
                        .SetSizeFromMid(chevronSize, chevronSize);
                    var buttonRect = Rect.zero.SetX(0).SetY(widgetRect_groupSpace.y).SetXMax(iconRect.xMax + 4)
                        .SetYMax(totalRect_groupSpace.yMax);
                ...
                void rightButton()
                {
                    ...
                    var iconRect = widgetRect_groupSpace.SetWidthFromRight(chevronOffset * 2)
                        .SetSizeFromMid(chevronSize, chevronSize);
                    var buttonRect = Rect.zero.SetX(iconRect.x - 6).SetY(widgetRect_groupSpace.y)
                        .SetXMax(totalRect_groupSpace.xMax).SetYMax(totalRect_groupSpace.yMax);
                ...
                void nameTextField()
                {
                    ...
                    var textFieldRect = widgetRect_groupSpace.AddHeightFromMid(-2)
                        .SetWidthFromMid(data.curPage.name.GetLabelWidth(textSize) + 4);
            ...
            void keys()
            {
                ...
                void selectByNumberKey()
                {
                    ...
                    if (i == lastNumberKeyPressedIndex &&
                        EditorApplication.timeSinceStartup - lastNumberKeyPressedTime_ticks < .3f)
                        OpenItem(data.curPage.items[i]);
                    else
                        SelectItem(data.curPage.items[i]);

                    lastNumberKeyPressedIndex = i;
                    lastNumberKeyPressedTime_ticks = DateTime.UtcNow.Ticks;
            ...
            void onOtherEvents()
            {
                ...
                if (totalRect_browserSpace.IsHovered())
                    if (curEvent.isMouseUp || curEvent.isMouseDrag ||
                        curEvent.isScroll) // prevents these events from reaching original gui
                        curEvent.Use();
            ...
            void originalBrowserGUI()
            {
                ...
                if (isOneColumn &&
                    currentOpacity
                        .Approx(1)) // to optimize locked one-column browser functioning as a dedicated favorites window
            ...
            if (isWrappedBrowserLocked)
                if (!totalRect_browserSpace.IsHovered() && !animatingDroppedItem && !animatingPageScroll)
                    return;
        ...
        static void SelectItem(Item item)
        {
            void openFolder(EditorWindow browser, string path)
            {
                ...
                else
                {
                    ...
                    browser.GetMemberValue("m_AssetTree")?.GetPropertyValue("data")?.InvokeMethod(
                        "SetExpanded",
                        folderAsset.GetInstanceID(),
                        true
                    );
                }
            }
            ...
            item.lastSelectTime_ticks = data.curPage.lastItemSelectTime_ticks = DateTime.UtcNow.Ticks;
        }
        ...
        {
            if (!shortcutPressed && !isWrappedBrowserLocked)
            {
                setDefaultState();
                return;
            }
            ...
            void down()
            {
                ...
                mousePressed = true;
                mousePressedOnCrossButtonArea = totalRect_browserSpace.SetWidthFromRight(0)
                    .MoveX(-crossButtonOffsetFromRight).SetWidthFromMid(crossButtonSize).IsHovered();
                ...
                if (pressedItemIndex.IsInRangeOf(data.curPage.items) && !mousePressedOnCrossButtonArea &&
        ...
        static void UpdateAnimations() // called from WrappedOnGUI  
        {
            ...
            {
                if (!curEvent.isLayout) return;
                if (!VFavoritesMenu.fadeAnimationsEnabled)
                {
            ...
            {
                if (!curEvent.isLayout) return;
                if (!VFavoritesMenu.pageScrollAnimationEnabled)
                {
                    pagesScrollPos = data.curPageIndex;
                    return;
                }
            ...
            void rowGaps()
            {
                ...
                for (int i = 0; i < data.curPage.rowGaps.Count; i++)
                    data.curPage.rowGaps[i] = Lerp(
                        data.curPage.rowGaps[i],
                        draggingItem && i == insertDraggedItemAtIndex ? rowHeight : 0,
                        lerpSpeed,
                        deltaTime
                    );
            }
            ...
            void droppedItem()
            {
                ...
                SmoothDamp(
                    ref droppedItemY_rowsSpace,
                    data.curPage.items.IndexOf(VFavorites.droppedItem) * rowHeight,
                    yLerpSpeed,
                    ref droppedItemYDerivative,
                    deltaTime
                );
        ...
        static void UpdateDragging() // called from WrappedOnGUI 
        {
            void initFromOutside()
            {
                ...
                if (draggingItemFromPageToOutside)
                    return; // to avoid duplication when dragging item from page to outside and back
                ...
                data.curPage.lastItemDragTime_ticks = DateTime.UtcNow.Ticks;
            }
            ...
            void initFromPage()
            {
                ...
                if (i < 0)
                    return; // somehow i = -1 with mouseDownPosition_rowsSpace.y = -20 when dragging in from outside quickly
        ...
        static void UpdateGUIWrapping() // called from EditorApplicaton.update 
        {
            void wrap()
            {
                ...
                if (t_VTabs != null && !EditorPrefs.GetBool("vFavorites-pluginDisabled", false) &&
                    EditorWindow.mouseOverWindow.GetMemberValue<bool>("isLocked")) return;
                ...
                t_BrowserWindow.SetFieldValue(
                    "s_LastInteractedProjectBrowser",
                    wrappedBrowser
                ); // so vTabs can copy its layout setting
            }
        ...
        static void WrapBrowserGUI(EditorWindow browser)
        {
            ...
            origBrowserOnGUIDelegate = fi_m_OnGUI.GetValue(hostView) as Delegate;
            fi_m_OnGUI.SetValue(hostView, newDelegate);
        }

        static void UnwrapBrowserGUI()
        {
            if (wrappedBrowser.GetFieldValue("m_Parent").GetFieldValue<Delegate>("m_OnGUI").Method.Name !=
                nameof(WrappedOnGUI)) return;
        ...
        static void UpdateLocking() // called from EditorApplicaton.update 
        {
            ...
            void lock_()
            {
                ...
                EditorPrefs.SetInt(
                    "vFavorites-lockedBrowserDockAreaInstanceId",
                    lockedBrowser.GetMemberValue<Object>("m_Parent").GetInstanceID()
                );
        ...
        static bool IsMarkedAsLocked(EditorWindow browser) =>
            browser.GetMemberValue("m_SearchFilter")?.GetMemberValue<string>("m_OriginalText") == "asd";

        static void MarkAsLocked(EditorWindow browser) =>
            browser.GetMemberValue("m_SearchFilter")?.SetMemberValue("m_OriginalText", "asd");

        static void MarkAsUnlocked(EditorWindow browser) =>
            browser.GetMemberValue("m_SearchFilter")?.SetMemberValue("m_OriginalText", "");
        ...
        static void Init()
        {
            ...
            void loadData()
            {
                data = AssetDatabase.LoadAssetAtPath<VFavoritesData>(
                    EditorPrefs.GetString("vFavorites-lastKnownDataPath-" + GetProjectId())
                );


                if (data) return;

                data = AssetDatabase.FindAssets("t:VFavoritesData")
                    .Select(guid => AssetDatabase.LoadAssetAtPath<VFavoritesData>(guid.ToPath())).FirstOrDefault();
